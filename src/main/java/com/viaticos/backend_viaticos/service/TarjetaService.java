...
public class TarjetaService {
    ...
    private TarjetaDTO mapToDTO(Tarjeta tarjeta) {
        ...
        if (tarjeta.getEmpleado() != null) {
            ...
            dto.setNombreEmpleado(tarjeta.getEmpleado().getNombre() + " " + tarjeta.getEmpleado().getApellido());
        }
        return dto;
    }
    ...
    public TarjetaDTO reportarTarjeta(Long idTarjeta, ReporteTarjetaDTO reporte) {
        ...
        if (tarjeta.getEmpleado() == null || !tarjeta.getEmpleado().getIdEmpleado().equals(reporte.getIdEmpleado())) {
            throw new RuntimeException("Operación no autorizada: Solo el titular puede reportar esta tarjeta.");
        }
